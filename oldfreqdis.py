...
def main(args):
    ...
    #########################################################################################
        # register handler
        relu_index = 0
        model_adv.img_encoder.blocks[-1].relu.register_forward_hook(get_features_hook)
        model_clip.img_encoder.blocks[-1].relu.register_forward_hook(get_features_hook_std)

    ###########################################################################################
        statis_results_robust_list = [] # Initialize as empty lists to hold batch results
        statis_results_std_list = []    # Initialize as empty lists to hold batch results
        magnitude_robust = []
        magnitude_std = []
        batch_idx = 0
        count_samples = 0
        for data_list, label in tqdm(dataloader_val):
            ...
            adv_data1 = attack.perturb(data, label, bool_unmasked_pos, 'mean', random_start=False, beta=1.0) # Changed 'mean' to 'max'
            ...
            if len(idx) > 0:
                ...
                count_activate_np = count_activate.cpu().numpy() # Ensure NumPy arrays
                feat_mean_magnitude_np = feat_mean_magnitude.cpu().numpy()

                statis_results_std_list.append(count_activate_np)     # Append NumPy arrays to lists
                magnitude_std.append(feat_mean_magnitude_np)
            ...
            if len(idx) > 0:
                ...
                count_activate_np = count_activate.cpu().numpy() # Ensure NumPy arrays
                feat_mean_magnitude_np = feat_mean_magnitude.cpu().numpy()
                statis_results_robust_list.append(count_activate_np)  # Append NumPy arrays to lists
                magnitude_robust.append(feat_mean_magnitude_np)
            batch_idx += 1
    ...
    #################################################################################
        ...
        # Concatenate lists of NumPy arrays into 2D NumPy arrays (batches x features)
        statis_results_robust_batches = np.stack(statis_results_robust_list, axis=0)
        statis_results_std_batches = np.stack(statis_results_std_list, axis=0)

        # Calculate the mean activation count across all batches (axis=0) for each feature (axis=1)
        mean_statis_results_robust = statis_results_robust_batches.mean(axis=0)
        mean_statis_results_std = statis_results_std_batches.mean(axis=0)

        res = np.stack([mean_statis_results_robust, mean_statis_results_std], axis=0) # Stack to get 2x196

        if not os.path.exists('./Frequency'): # More robust path creation
            os.makedirs('./Frequency', exist_ok=True) # Use exist_ok for safety
