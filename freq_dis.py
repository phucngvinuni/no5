...
def main(args): 
    ...
    attack = FastGradientSignUntargeted(model_adv, 8./255., 2./255., min_val=0, max_val=1, max_iters=4)
    ...
    ###########################################################################################
        ...
        for data_list, label in tqdm(dataloader_val):
            ...
            adv_data1 = attack.perturb(data, label, bool_unmasked_pos, 'mean', False, 1.)
    ...
    #################################################################################
        ...
        Path('Frequency').mkdir(parents=True, exist_ok=True)
        np.save('./Frequency/cifar10_std_class1.npy', res)
